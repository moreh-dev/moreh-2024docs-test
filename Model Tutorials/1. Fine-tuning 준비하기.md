...
만약 `conda: command not found` 메시지가 표시되거나, torch 패키지가 리스트되지 않거나, 혹은 torch 패키지가 존재하더라도 버전명에 “moreh”가 포함되지 않은 경우 ***([Prepare Fine-tuning on MoAI Platform](../Supported%20Documents/Prepare%20Fine-tuning%20on%20MoAI%20Platform.md))*** 문서에 따라 conda 환경을 생성하십시오.
